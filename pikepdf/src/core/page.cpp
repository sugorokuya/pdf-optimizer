    ...
    py::class_<QPDFPageObjectHelper, py::smart_holder, QPDFObjectHelper>(m, "Page")
        ...
        .def(
            ...
            [](QPDFPageObjectHelper &poh,
                ...
                auto resources = poh.getAttribute("/Resources", true);
                if (!resources.isDictionary())
                    throw py::value_error("Page has no resources dictionary");
                    
                auto xobject = resources.getKey("/XObject");
                if (!xobject.isDictionary())
                    throw py::value_error("Page has no XObject dictionary");
                    
                // Find the old image in XObject dictionary
                QPDFObjectHandle found_image;
                std::string image_name;
                for (auto const& [name, obj] : xobject.ditems()) {
                    if (obj.getObjGen() == old_image.getObjGen()) {
                        found_image = obj;
                        image_name = name;
                        break;
                    }
                ...
                if (found_image.isNull())
                    throw py::value_error("Image not found in page resources");
                    
                // Create new image stream with JPEG data
                std::string jpeg_data(new_jpeg_data);
                auto filter = QPDFObjectHandle::newName("/DCTDecode");
                found_image.replaceStreamData(jpeg_data, filter, QPDFObjectHandle::newNull());
                
                // Handle SMask if provided
                if (!new_smask_data.is_none() && py::len(new_smask_data) > 0) {
                    std::string smask_data(new_smask_data);
                    
                    // Get or create SMask object
                    auto smask = found_image.getDict().getKey("/SMask");
                    if (smask.isNull() || !smask.isStream()) {
                        // Create new SMask stream object
                        auto owner = poh.getObjectHandle().getOwningQPDF();
                        if (!owner)
                            throw py::value_error("Page is not attached to a Pdf");
                            
                        smask = QPDFObjectHandle::newStream(owner);
                        found_image.getDict().replaceKey("/SMask", smask);
                    }
                    
                    // Update SMask stream data
                    smask.replaceStreamData(smask_data, filter, QPDFObjectHandle::newNull());
                    
                    // Set SMask properties
                    smask.getDict().replaceKey("/Type", QPDFObjectHandle::newName("/XObject"));
                    smask.getDict().replaceKey("/Subtype", QPDFObjectHandle::newName("/Image"));
                    smask.getDict().replaceKey("/ColorSpace", QPDFObjectHandle::newName("/DeviceGray"));
                    smask.getDict().replaceKey("/BitsPerComponent", QPDFObjectHandle::newInteger(8));
                    
                    // Copy dimensions from main image
                    auto width = found_image.getDict().getKey("/Width");
                    auto height = found_image.getDict().getKey("/Height");
                    if (!width.isNull()) smask.getDict().replaceKey("/Width", width);
                    if (!height.isNull()) smask.getDict().replaceKey("/Height", height);
                }
